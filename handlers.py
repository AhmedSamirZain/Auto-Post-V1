...
def _parse_cairo_time(text: str):
    """يفهم مواعيد عربية بسيطة بجانب الصيغ الرقمية.

    أمثلة مدعومة:
    - 14:30
    - 2026-06-01 14:30
    - غداً 10 الصبح / بكرة 8 مساءً / بعد بكرة 11:45
    - الساعة 7 م / 9 صباحا
    """
    if not text:
        return None

    def _to_en_digits(v: str) -> str:
        ar = "٠١٢٣٤٥٦٧٨٩۰۱۲۳۴۵۶۷۸۹"
        en = "01234567890123456789"
        return str(v).translate(str.maketrans(ar, en))

    raw = _to_en_digits(text).strip().lower()
    raw = raw.replace("ـ", " ")
    raw = re.sub(r"\s+", " ", raw)
    now = _cairo_now()

    # YYYY-MM-DD HH:MM
    m2 = re.search(r'(\d{4}-\d{1,2}-\d{1,2})\s+(\d{1,2})(?::(\d{2}))?', raw)
    if m2:
        try:
            mi = int(m2.group(3) or 0)
            naive = datetime.strptime(f"{m2.group(1)} {int(m2.group(2)):02d}:{mi:02d}", "%Y-%m-%d %H:%M")
            return CAIRO_TZ.localize(naive)
        except Exception:
            pass

    day_offset = None
    if any(w in raw for w in ("بعد بكرة", "بعد غد", "بعد غداً", "بعد غدا")):
        day_offset = 2
    elif any(w in raw for w in ("غدا", "غداً", "بكره", "بكرة")):
        day_offset = 1
    elif any(w in raw for w in ("اليوم", "النهارده", "النهاردة", "دلوقتي")):
        day_offset = 0

    # HH:MM أو ساعة مفردة مع دلالات صباح/مساء
    m = re.search(r'(?:الساعة|ساعه|at)?\s*(\d{1,2})(?::(\d{2}))?', raw)
    if not m:
        return None
    ...
    is_pm = bool(re.search(r'(^|\s)(م|pm)(\s|$)', raw)) or any(
        w in raw for w in ("مساء", "مساءً", "بليل", "بالليل", "ليل")
    )
    is_am = bool(re.search(r'(^|\s)(ص|am)(\s|$)', raw)) or any(
        w in raw for w in ("صباح", "صباحاً", "الصبح", "فجر")
    )
    if h <= 12:
        if is_pm and h < 12:
            h += 12
    ...
    # لو المستخدم كتب وقت فقط أو "اليوم" والوقت فات، نخليه بكرة
    if (day_offset is None or day_offset == 0) and dt <= now:
        dt += timedelta(days=1)
    return dt
...
async def _save_account(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        name = await FBAutomator(0, cookies, proxy).get_account_name()
    except Exception:
...
async def cb_acc_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _edit(update,
        ...
        ik(
            [btn("🗂 سحب المجموعات",   f"acc_fetch_grp_{acc_id}"),
             btn("📄 سحب الصفحات",     f"acc_fetch_pg_{acc_id}")],
            [btn("🔄 تحديث الاسم",      f"acc_refresh_name_{acc_id}"),
             btn("➕ صفحة يدوياً",      f"pg_manual_acc_{acc_id}")],
...
async def cb_acc_refresh_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """يعيد محاولة جلب اسم الحساب الحقيقي من فيسبوك ثم يحدثه في قاعدة البيانات."""
    await _answer(update)
    acc_id = int(update.callback_query.data.split("_")[-1])
    uid = update.effective_user.id
    accs = await db.get_accounts(uid)
    ...
    if not acc:
        return S_ACCOUNTS
    await _edit(update, "⏳ *جاري تحديث اسم الحساب من فيسبوك...*")
    from fb_automator import FBAutomator
    try:
        name = await FBAutomator(acc_id, acc.get("cookies", ""), acc.get("proxy")).get_account_name()
    except Exception as e:
        name = ""
        logger.warning(f"refresh account name failed: {e}")
    c_user = _extract_cuser(acc.get("cookies", "[]"))
    if not name or name == f"حساب {c_user}":
        await _edit(update,
            "⚠️ *لم أستطع جلب الاسم الحقيقي تلقائياً.*\n\n"
            "ده غالباً بسبب قيود فيسبوك/الكوكيز أو عدم اكتمال الصلاحيات.\n"
            "تقدر تسيبه بالمعرف أو تحذف الحساب وتعيد ربطه بكوكيز أحدث.",
            ik(back_btn("accounts"))
        )
        return S_ACCOUNTS
    await db.update_account_name(acc_id, uid, name)
    await _edit(update, f"✅ *تم تحديث الاسم:*\n{_escape_md(name)}", ik(back_btn("accounts")))
    return S_ACCOUNTS
...
async def cb_acc_fetch_pg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if pages:
        ...
        await _edit(update,
            "⚠️ *لم يُعثر على صفحات تلقائياً.*\n\n"
            "لو الحساب فعلاً أدمن/مالك صفحات، جرّب تحديث الكوكيز أو أضف الصفحة يدوياً.",
            ik([btn("➕ إضافة صفحة يدوياً", f"pg_manual_acc_{acc_id}")], back_btn("accounts"))
        )
    return S_ACCOUNTS


async def cb_pg_manual(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """إضافة صفحة يدوياً عند فشل السحب التلقائي."""
    await _answer(update)
    uid = update.effective_user.id
    data = update.callback_query.data if update.callback_query else "pg_manual"
    acc_id = None
    if data.startswith("pg_manual_acc_"):
        try:
    ...
    if acc_id is None:
        ...
        if not accs:
            ...
            return S_PAGES
        acc_id = accs[0]["id"]
    context.user_data["manual_page_acc_id"] = acc_id
    await _edit(update,
        "➕ *إضافة صفحة يدوياً*\n━━━━━━━━━━━━━━━━━━\n\n"
        "أرسل رابط الصفحة أو المعرف، ويمكنك كتابة الاسم في السطر الثاني.\n\n"
        "مثال:\n"
        "`https://www.facebook.com/MyPage`\n"
        "`اسم الصفحة`\n\n"
        "أو:\n"
        "`123456789 | اسم الصفحة`",
        ik(back_btn("pages"))
    )
    return S_PAGE_MANUAL


def _parse_manual_page_text(text: str):
    text = (text or "").strip()
    if not text:
        return None, None
    # صيغة id | name
    if "|" in text:
        ...
        if left:
            return left, right or left
    lines = [l.strip() for l in text.splitlines() if l.strip()]
    first = lines[0]
    name = lines[1] if len(lines) > 1 else ""
    pid = first
    m = re.search(r'profile\.php\?id=(\d+)', first)
    if m:
        pid = m.group(1)
    else:
        m = re.search(r'(?:facebook\.com|fb\.com)/([^/?#&]+)', first)
        if m:
            pid = m.group(1)
    pid = pid.strip().strip('/')
    if not name:
        name = pid.replace('.', ' ').replace('_', ' ').replace('-', ' ').strip() or "صفحة"
    return pid, name


async def pg_got_manual(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    acc_id = context.user_data.get("manual_page_acc_id")
    if not acc_id:
        accs = await db.get_accounts(uid)
        if not accs:
            ...
            return S_PAGES
        acc_id = accs[0]["id"]
    pid, name = _parse_manual_page_text(update.message.text)
    if not pid:
        await _send(update, "❌ لم أفهم الصفحة. أرسل رابط الصفحة أو ID الصفحة.")
        return S_PAGE_MANUAL
    await db.add_page(uid, acc_id, pid, name)
    context.user_data.pop("manual_page_acc_id", None)
    await db.log_activity(uid, "add_page_manual", f"{name} ({pid})")
    await _send(update,
        f"✅ *تم حفظ الصفحة يدوياً!*\n\n"
        f"📄 الاسم: *{_escape_md(name)}*\n"
        f"🆔 المعرف/الرابط: `{_escape_md(pid)}`\n\n"
        "يمكنك الآن استخدامها في النشر والستوري.",
        inline_kb=ik([btn("📄 الصفحات", "pages")], back_btn("accounts"))
    )
    return S_PAGES
...
async def _show_pages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    # تنظيف أي تدفق جدولة صفحات تم إلغاؤه
    for k in ("page_post_schedule", "page_post_schedule_dt"):
...
async def _show_page_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not pages:
        await _send(update,
            "📄 *نشر الصفحات*\n\n⚠️ لا توجد صفحات مربوطة.\nاسحب صفحاتك أولاً من قسم الحسابات أو أضف صفحة يدوياً.",
            inline_kb=ik([btn("👤 الحسابات","accounts")], [btn("➕ إضافة صفحة يدوياً", "pg_manual")])
        )
        return S_PAGES
...
def _persist_scheduled_media(media_path: str, uid: int, prefix: str = "story") -> str:
    """ينقل/ينسخ ملف مؤقت إلى data/scheduled_media حتى لا تحذفه صيانة /tmp قبل الموعد."""
    if not media_path or not os.path.exists(media_path):
        return media_path
    # لو الملف محفوظ بالفعل داخل data نرجعه كما هو
    if os.path.abspath(media_path).startswith(os.path.abspath(os.path.join(os.path.dirname(__file__), "data"))):
        return media_path
    import shutil
    media_dir = os.path.join(os.path.dirname(__file__), "data", "scheduled_media")
    os.makedirs(media_dir, exist_ok=True)
    ext = os.path.splitext(media_path)[1] or ".bin"
    dst = os.path.join(media_dir, f"{prefix}_{uid}_{secrets.token_hex(8)}{ext}")
    shutil.copy2(media_path, dst)
    try:
        if media_path.startswith("/tmp/"):
...
async def _schedule_story(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """يحفظ الستوري للنشر المجدول بدل نشره فوراً."""
    uid = update.effective_user.id
    accs = await db.get_accounts(uid)
    if not accs:
        await _edit(update, "⚠️ يجب ربط حساب أولاً!", ik([btn("👤 الحسابات","accounts")]))
    ...
    if not sel:
        ...
        return S_PAGES
    dt = context.user_data.get("story_schedule_dt")
    if not dt:
        txt = context.user_data.get("story_time")
        dt = _parse_cairo_time(txt) if txt else None
    if not dt:
        await _edit(update, "❌ لم أجد موعداً صالحاً للجدولة.", ik([btn("✏️ اختيار موعد", "story_edit")], back_btn("pages")))
        return S_PAGE_STORY_LINK

    media_path = context.user_data.get("story_img")
    media_path = _persist_scheduled_media(media_path, uid, "story") if media_path else None
    story_id = await db.add_scheduled_story(
        user_id=uid,
        account_id=accs[0]["id"],
        page_ids=sel,
        media_path=media_path,
        story_type=context.user_data.get("story_type", "image"),
    ...
    await db.log_activity(uid, "جدولة ستوري", f"ستوري #{story_id} على {len(sel)} صفحة في {dt:%Y-%m-%d %H:%M}")
    for k in ("story_img","story_link","story_pages","story_type","story_text","story_time","story_schedule_dt"):
        context.user_data.pop(k, None)
    await _send(update,
        f"✅ *تم جدولة الستوري #{story_id} بنجاح!*\n━━━━━━━━━━━━━━━━━━\n\n"
        f"📅 الموعد: *{readable}*\n"
        f"📄 الصفحات: *{len(sel)}*\n"
        f"🕐 التوقيت: القاهرة",
...
async def _do_post_story(update, context):
    ...
    for i, pg in enumerate(target_pages):
        try:
            res = await auto.post_story(pg.get("page_id",""), img, link, story_text)
        except Exception as e:
...
async def story_got_time(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not dt:
        ...
        return S_PAGE_STORY_LINK
    readable = _format_cairo_dt(dt)
    context.user_data["story_time"] = dt.strftime("%Y-%m-%d %H:%M")
    context.user_data["story_schedule_dt"] = dt
    await _send(update,
        f"🕐 *فهمت الموعد هكذا:*\n\n"
        f"📅 {readable}\n"
        f"(توقيت القاهرة)\n\n"
...
async def cb_story_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await _answer(update)
    if context.user_data.get("story_schedule_dt") or context.user_data.get("story_time"):
        await _edit(update, "⏳ *جاري حفظ جدولة الستوري...*", ik())
        return await _schedule_story(update, context)
...
async def _launch_page_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not target_pages:
        ...
        return S_PAGES
    if context.user_data.get("page_post_schedule") and not context.user_data.get("page_post_schedule_dt"):
        now_c = _cairo_now()
        await _send(update,
            f"⏰ *موعد نشر الصفحات*\n━━━━━━━━━━━━━━━━━━\n\n"
            f"اكتب وقت النشر بتوقيت القاهرة.\n"
            f"الوقت الحالي: *{now_c.strftime('%H:%M')}*\n\n"
            f"أمثلة: `14:30` أو `غداً 10 الصبح`",
...
async def _run_page_campaign_bg(context, uid, camp_id, acc, page_ids, caption, media_path, delay=60):
    """تشغيل نشر صفحات مجدول/خلفي مع تحديث حالة الحملة."""
    from fb_automator import FBAutomator
    await db.update_campaign_status(camp_id, "running")
    pages = await db.get_pages(uid)
    targets = [p for p in pages if p["id"] in page_ids]
    auto = FBAutomator(acc["id"], acc.get("cookies", ""), acc.get("proxy"))
    done = failed = 0
    for i, pg in enumerate(targets):
        ...
        await asyncio.sleep(random.uniform(max(delay - 20, 5), delay + 20))
    await db.update_campaign_status(camp_id, "done" if done else "failed", done)
    await db.log_activity(uid, "نشر صفحات مجدول", f"حملة #{camp_id}: ✅{done} ❌{failed}", "success" if done else "error")
    try:
        if media_path and os.path.exists(media_path):
            base = os.path.abspath(os.path.join(os.path.dirname(__file__), "data", "scheduled_media"))
            if os.path.abspath(media_path).startswith(base) or media_path.startswith("/tmp/"):
                os.remove(media_path)
    except Exception:
        pass
    await _notify_user(context, uid, f"✅ *انتهى نشر الصفحات #{camp_id}!*\n\n✅ نجح: {done}\n❌ فشل: {failed}")
...
async def camp_got_schedule(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get("page_post_schedule"):
        return await pagepost_got_schedule(update, context)
...
async def pagepost_got_schedule(update: Update, context: ContextTypes.DEFAULT_TYPE):
    dt = _parse_cairo_time(update.message.text.strip())
    if not dt:
        await _send(update,
            "❌ *صيغة غير مفهومة.*\n\n"
            "استخدم مثلاً: `14:30` أو `غداً 10 الصبح`"
        )
        return S_CAMP_SCHEDULE
    context.user_data["page_post_schedule_dt"] = dt
    readable = _format_cairo_dt(dt)
    sel = context.user_data.get("pg_selected", [])
    await _send(update,
        f"🕐 *فهمت موعد نشر الصفحات هكذا:*\n\n"
        f"📅 {readable}\n"
        f"📄 الصفحات: *{len(sel)}*\n"
        f"(توقيت القاهرة)\n\n"
        f"هل تريد تأكيد هذا الموعد؟",
        inline_kb=ik(
            [btn("✅ تأكيد الموعد", "ppost_sched_confirm")],
            [btn("✏️ تعديل الموعد", "ppost_sched_edit")],
            [btn("❌ إلغاء", "pages")],
        )
    )
    return S_CAMP_SCHEDULE
...
async def cb_ppost_sched_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await _answer(update)
    context.user_data.pop("page_post_schedule_dt", None)
    await _edit(update,
        "✏️ *أرسل موعد نشر الصفحات من جديد:*\n\n"
        "مثال: `14:30` أو `غداً 10 الصبح`",
        ik([btn("❌ إلغاء", "pages")])
    )
    return S_CAMP_SCHEDULE


async def cb_ppost_sched_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await _answer(update)
    uid = update.effective_user.id
    dt = context.user_data.get("page_post_schedule_dt")
    camp = context.user_data.get("camp", {})
    sel = context.user_data.get("pg_selected", [])
    if not dt or not sel:
        await _edit(update, "❌ بيانات الجدولة ناقصة. ابدأ من جديد.", ik(back_btn("pages")))
        return S_PAGES
    accs = await db.get_accounts(uid)
    if not accs:
        await _edit(update, "❌ يجب ربط حساب أولاً!", ik([btn("👤 الحسابات", "accounts")]))
    ...
    media_path = camp.get("media_path")
    if media_path:
        media_path = _persist_scheduled_media(media_path, uid, "pagepost")
    camp_id = await db.add_campaign(
        uid, accs[0]["id"],
        title=f"نشر صفحات {datetime.now().strftime('%m-%d %H:%M')}",
        content=camp.get("caption", ""),
        targets=sel,
        target_type="pages",
        media_path=media_path,
    ...
    readable = _format_cairo_dt(dt)
    for k in ("page_post_schedule", "page_post_schedule_dt", "pg_selected", "pg_dist"):
        context.user_data.pop(k, None)
    context.user_data["camp"] = {}
    await _edit(update,
        f"✅ *تم جدولة نشر الصفحات #{camp_id}*\n━━━━━━━━━━━━━━━━━━\n\n"
        f"📅 الموعد: *{readable}*\n"
        f"📄 الصفحات: *{len(sel)}*",
        ik([btn("🗂 السجل", "camp_log_cb")], back_btn("pages"))
    )
    return S_PAGES
...
async def cb_ppost_schedule(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """جدولة نشر الصفحات."""
    await _answer(update)
    context.user_data["page_post_schedule"] = True
    context.user_data["pg_selected"] = []
    await _edit(update,
        "⏰ *جدولة نشر الصفحات*\n━━━━━━━━━━━━━━━━━━\n\n"
        "اختر الصفحات، ثم أرسل الميديا والكابشن، وبعدها سأطلب موعد النشر.",
    ...
    return ConversationHandler(
        ...
        states={
            ...
            S_PAGES: shared_cbs + [
                ...
                CallbackQueryHandler(cb_pg_manual,       pattern=r"^pg_manual(_acc_\d+)?$"),
            ],
            ...
            S_PAGE_MANUAL: [
                MessageHandler(nav_filter, nav_router),
                MessageHandler(any_text, pg_got_manual),
            ...
            S_CAMP_SCHEDULE: [
                ...
                CallbackQueryHandler(cb_ppost_sched_confirm, pattern="^ppost_sched_confirm$"),
                CallbackQueryHandler(cb_ppost_sched_edit,    pattern="^ppost_sched_edit$"),
