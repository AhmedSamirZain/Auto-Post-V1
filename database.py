...
async def add_scheduled_story(user_id: int, account_id: int, page_ids: list,
                              media_path: str = None, story_type: str = "image",
                              story_text: str = "", link: str = None,
                              schedule_time: str = None) -> int:
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            """INSERT INTO scheduled_stories
               (user_id, account_id, page_ids, media_path, story_type, story_text, link, schedule_time)
               VALUES (?,?,?,?,?,?,?,?)""",
            (user_id, account_id, json.dumps(page_ids or []), media_path, story_type,
             story_text or "", link, schedule_time)
        )
        await db.commit()
        return cur.lastrowid
...
async def get_pending_stories():
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT * FROM scheduled_stories WHERE status = 'pending' ORDER BY schedule_time ASC"
        ) as cur:
            return [dict(r) for r in await cur.fetchall()]


async def update_scheduled_story_status(story_id: int, status: str,
                                        done: int = None, failed: int = None):
    async with aiosqlite.connect(DB_PATH) as db:
        fields = ["status = ?"]
        vals = [status]
        if done is not None:
        ...
        await db.execute(
            f"UPDATE scheduled_stories SET {', '.join(fields)} WHERE id = ?",
            tuple(vals)
        )
        await db.commit()


async def get_scheduled_stories(user_id: int, limit: int = 20):
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT * FROM scheduled_stories WHERE user_id = ? ORDER BY created_at DESC LIMIT ?",
            (user_id, limit)
        ) as cur:
            return [dict(r) for r in await cur.fetchall()]
